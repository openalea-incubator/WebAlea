 ...
 * FlowContextV2.jsx
 *
 * Version améliorée du FlowContext avec intégration du WorkflowEngineV2.
 *
 * Changements par rapport à FlowContext.jsx:
 * - Utilise WorkflowEngineV2 avec gestion des dépendances
...
import { WorkflowEngineV2, NodeState } from '../engine/WorkflowEngineV2.jsx';
...
export const FlowProviderV2 = ({ children }) => {
    ...
    /**
     * Handler pour les événements du moteur V2
     */
    const handleEngineEvent = useCallback((event, payload) => {
        console.log("WorkflowEngineV2 event:", event, payload);

        switch (event) {
            case "workflow-start":
                console.log("Workflow V2 started with", payload.totalNodes, "nodes");
                ...
                addLog("Workflow execution started (V2)", { totalNodes: payload.totalNodes });
                break;
            ...
            case "workflow-done":
                if (payload.success) {
                    console.log("Workflow V2 completed successfully");
                    setExecutionStatus('completed');
                    addLog("Workflow completed successfully", {
                        results: Object.keys(payload.results).length
                    });
                } else {
                    console.log("Workflow V2 completed with errors");
            ...
            case "workflow-error":
                console.error("Workflow V2 failed:", payload.error);
                setExecutionStatus('failed');
    ...
    /**
     * Exécute le workflow complet avec le moteur V2
     */
    const executeWorkflow = useCallback(async () => {
        const { graph, edges: customEdges } = buildGraphModel(nodes, edges);

        console.log("Executing workflow V2 with:", {
            nodes: graph.length,
