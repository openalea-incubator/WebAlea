...
export class WorkflowEngineV2 {
    ...
    async start() {
        if (this.running) {
            console.warn("WorkflowEngineV2: Already running");
            return { success: false, error: "Already running" };
        }

        if (!this.graph || this.graph.length === 0) {
            console.warn("WorkflowEngineV2: No model bound");
        ...
        console.log("WorkflowEngineV2: Starting with", this.graph.length, "nodes");

        try {
            // Trouver les nodes initialement prêts (roots)
            const initialReady = this.dependencyTracker.getReadyNodes();
            console.log("WorkflowEngineV2: Initial ready nodes:", initialReady);
            ...
            if (unfinished.length > 0) {
                console.warn("WorkflowEngineV2: Nodes not executed:", unfinished);
            }
            ...
            console.error("WorkflowEngineV2: Workflow failed:", error);
    ...
    stop() {
        ...
        console.log("WorkflowEngineV2: Stopping workflow");
        this.running = false;
    ...
    async _executeNode(nodeId) {
        ...
        try {
            ...
            console.log(`WorkflowEngineV2: Executing ${nodeId} (${node.label}) with inputs:`,
            ...
            if (newlyReady.length > 0 && this.running) {
                console.log(`WorkflowEngineV2: Nodes now ready after ${nodeId}:`, newlyReady);
            ...
            console.error(`WorkflowEngineV2: Node ${nodeId} failed:`, error);
    ...
    async _executeViaBackend(node, inputs) {
        console.log(`WorkflowEngineV2: Backend call for ${node.id}`, {
    ...
    _emit(event, payload) {
        for (const listener of this.listeners) {
            try {
                ...
                console.error("WorkflowEngineV2: Listener error:", e);
            }
        }
    }
}
